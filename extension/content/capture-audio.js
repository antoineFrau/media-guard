...
(function () {
  ...
  window.MediaGuardCapture = {
    ...
    async startCapture(options) {
      const { videoId, apiBaseUrl, mistralKey, elevenlabsKey, onStatus, onTranscript, onComplete, onError, onCaptureStart, videoElement } = options;
      ...
      try {
        ...
        if (typeof captureFn !== 'function') {
          onError('Audio capture not supported in this browser. Try Chrome, or use the STT app at localhost:8000 with Tab + ElevenLabs.');
          return;
        }
        ...
        async function runAnalyze() {
          ...
          try {
            const res = await fetch(`${apiBaseUrl}/analyze`, {
              ...
              body: JSON.stringify({ video_id: videoId, transcript, mistral_api_key: mistralKey })
            });
