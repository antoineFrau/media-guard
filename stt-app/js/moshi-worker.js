...
async function fetchArrayBuffer(url) {
  const cacheName = "mediaguard-stt-cache";
...
class MoshiASR {
  ...
  static async initialize(params) {
    ...
    try {
      ...
      self.postMessage({ status: "loading", message: "Loading Model (~950 MB)" });
  ...
  static startStream() {
    if (this.decoder) this.decoder.start_streaming();
  }

  static stopStream() {
    if (this.decoder) this.decoder.stop_streaming();
  }

  static processAudio(audioData) {
    if (this.decoder) {
      this.decoder.process_audio_chunk(audioData, (word) => {
        self.postMessage({ status: "streaming", word });
      });
      self.postMessage({ status: "chunk_processed" });
    }
  }
}
...
self.addEventListener("message", async (event) => {
  ...
  switch (command) {
    case "initialize":
      ...
      await MoshiASR.initialize({ weightsURL, tokenizerURL, mimiURL, configURL });
      break;
    case "start_stream":
    ...
    case "stop_stream":
      ...
      break;
    case "process_audio":
      MoshiASR.processAudio(event.data.audioData);
      break;
  }
});
